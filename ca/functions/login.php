...
<?php
	if(isset($_POST['username']) && isset($_POST['password'])){
		if($_POST['username']!= NULL and $_POST['password']!= NULL){
			...
			$result =$ca->ca_login($user_name,$password);
		}
	}
	if($result==0){
		session_start();		
		$_SESSION['ca_user_id']=$ca->id;
		/*$query="INSERT INTO `loged_users`(`user_id`, `ip_address`) VALUES (".$_SESSION['ca_user_id'].",'".get_client_ip()."')";
