		...
		isset($_POST['password']) 	 ){
			...
			if( !empty($fullname) and
				...
				!empty($password)  ){
					...
					if($result){	
						//$_SESSION['ca_user_id']=$user->id;
						sendmail($email,$fullname,$key);					
...
function sendmail($email,$name){
	$subject='welcome to socilize ';
	$text='Hi '.$name."\n".'welcome to socilize, '/*'Your mla on click  user verification key is :'. $key */."\n".
		'this mail is send to '.$name.'  if you are not suppose to receive this mail just discard this mail';
	$header='From : Team  socilize <no-reply@socilize.com>';
