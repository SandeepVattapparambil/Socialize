...
class ca {
    ...
    public $ca_mob_no; 
    public $ca_email;
    public $ca_designation;
	public $user_image;
        private $password;
	
	
	public function ca_login($_usr,$_pass){
	$query="SELECT  `id`,`password`,`password_salt` FROM `central_authority` WHERE `ca_mail_id` = '".$_usr."'";
	...
	public function get_ca($_user_id=0){
		if($_user_id!=0){
			$this->id=$_user_id;
		}
		$query="SELECT * FROM `central_authority` WHERE `id`=".$this->id;
		$result=execute_query($query);
   		$this->ca_name			= $result[0]['ca_name'];		
		$this->ca_mob_no		= $result[0]['ca_mob'];
		$this->ca_email			= $result[0]['ca_mail_id'];
		$this->id				= $result[0]['id'];
		$this->ca_designation	= $result[0]['ca_designation'];
		$this->user_image	= $result[0]['user_image'];
                $this->password         = $result[0]['password'];
	}
	
        public function edit_ca($name,$mobile,$email){
        //$_date_of_birth=date('Y-m-d',$_date_of_birth_time_stamp);
		//$_date_of_registration=date('Y-m-d',$_date);
        $result=false;        
        $query="UPDATE `central_authority` SET `ca_name`='".$name."',`ca_mob`='".$mobile."',`ca_mail_id`='".$email."' WHERE `id`='".$this->id."'";
		$result=execute_update($query); 
		$this->ca_name	= $name;
		$this->ca_mob_no = $mobile;
		$this->ca_email	= $email;
		return $result;
    }
        ...
        {
            $result=false;
            $query="SELECT `password_salt` FROM `central_authority` WHERE `id`=".$this->id;
            ...
            if($calc_password==$this->password)
            {
                ...
                $query="UPDATE `central_authority` SET `password`='".$new_calc_password."' WHERE `id`=".$this->id;
                $result=  execute_update($query);
