...
<?php
	if(isset($_POST['username']) && isset($_POST['password'])){
		if($_POST['username']!= NULL and $_POST['password']!= NULL){
			$admin	 = new Admin();		
			$admin_name=	strtolower(htmlentities($_POST['username']));
			$password=htmlentities($_POST['password']);
			$result =$admin->login($admin_name,$password);
		}
	}
	if($result==0){
		session_start();		
		$_SESSION['database_admin']=$admin->id;
		/*$query="INSERT INTO `loged_users`(`user_id`, `ip_address`) VALUES (".$_SESSION['database_admin'].",'".get_client_ip()."')";
		execute_update($query);
		$result=execute_query("SELECT `id` FROM `loged_users` WHERE `user_id`=".$admin->id." ORDER BY `id` DESC limit 1");
