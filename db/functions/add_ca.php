...
if(	//!empty($_POST['id'])		and 
	...
	$type		=$_POST['type']			;
		...
		$image=addslashes($image);
			...
			$sql = "INSERT INTO `central_authority`(`ca_name`, `ca_designation`, `ca_mail_id`, `user_image`, `password`, `password_salt`) VALUES ('$name','$designation','$email','$image','$password','$password_salt')";
			if(execute_update($sql)){
				$sql = "SELECT `id` FROM `central_authority` WHERE `ca_mail_id`='$email'";
			...
			if(	isset($_POST['id']) and !empty($_POST['id'])){
				$id 		=$_POST['id'];
				$sql="UPDATE `central_authority` SET `ca_name`='$name',`ca_designation`='$designation' WHERE `id`=$id";
				if(execute_update($sql)){
					row($id,$name,$designation,$email);
				}else 'error';
			}else 'error';
