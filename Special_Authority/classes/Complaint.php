...
class Complaint {
    ...
    public function getComplaintList($_sa_id){
        $result = array(); 
		$query="SELECT `id` FROM `complaints` WHERE `position`='sa' and `position_id`=$_sa_id and `status` !=4 ORDER BY `time_stamp` DESC";
	...
	public function getClearedComplaintList($_sa_id){
        $result = array(); 
		$query="SELECT `id` FROM `complaints` WHERE `position`='sa' and `position_id`=$_sa_id and `status` =4 ORDER BY `time_stamp` DESC";
		$result_id = execute_query($query);
		$i=0;
	...
	public function getLatestComplaintList($_sa_id){
        $result = array(); 
		$query="SELECT `id` FROM `complaints` WHERE `position`='sa' and `position_id`=$_sa_id and `status` !=4 ORDER BY `time_stamp` DESC Limit 10";
		$result_id = execute_query($query);
		$i=0;
		foreach($result_id as $element){
			$result[$i]=$element['id'];
