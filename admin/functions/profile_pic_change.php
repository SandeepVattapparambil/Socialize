    ...
    */
    ...
    //uploading image
    $img=upload_profile_image('image');
    
    if($img==41)
        header("location: ../profile.php?img_msg=2"); //image size exceeded
    else if($img==42)
        header("location: ../profile.php?img_msg=3"); //image format error
    else
        header("location: ../profile.php?img_msg=1"); //success
    
    function upload_profile_image($fn){
    $msg=0;
    global $admin;
    if (isset($_FILES[$fn])){
            if($_FILES[$fn]['error']==0){
                    $image= move_profile_image($fn);	
                    if (!empty($image) and $image!=40 and $image!=41 and $image!=42 ){
                            ...
                            if(@imagecreatefromstring($image_file)){
                                    ...
                                    $admin->add_admin_image($image_file);
                                    fclose($handle);
                                    unlink($image);
    ...
    function move_profile_image($fn){
